...
(defmacro defn
  ...
  (let [m (if (string? (first fdecl))
    ...
    (if (and (sequential? events)
      ...
      `(let [event-fn# (fn [cofx# ~@args]
                         (if (and (map? cofx#)
                                  (not (nil? (:db cofx#))))
                           (let [~cofx cofx#]
                             ~@fdecl)
                           (throw (js/Error. (str "fx/defn expects a map of cofx as first argument got "
                                                  cofx# "in function " ~(clojure.core/name name))))))]
         (clojure.core/defn ~(with-meta name m)
           ([~@args] (fn [cofx#] (event-fn# cofx# ~@args)))
           ([cofx# ~@args] (event-fn# cofx# ~@args)))
         ~@(map (fn [event]
                  `(status-im.utils.handlers/register-handler-fx
                    ~event
                    (fn [cofx# [_ ~@args]] (event-fn# cofx# ~@args))))
                events))
